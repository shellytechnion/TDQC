...
def load_rollouts_from_root(load_root: Path, cfg: Config) -> list[Rollout]:
    ...
    for env_record_path in env_record_paths:
        ...
        for policy_record in policy_records:
            ...
            if cfg.dataset.max_feat > 0:
                top_indices = np.argsort(hidden_state, axis=-1)[..., -cfg.dataset.max_feat:]  # (n_tokens, max_feat)
                # Extract top probabilities
                hidden_state = np.take_along_axis(hidden_state, top_indices, axis=-1)  # (n_tokens, max_feat)
