...
def compute_au_eu_metrics(
    token_prob: np.ndarray,
    token_entropy: np.ndarray,
    top_10_probs: np.ndarray = None,
) -> np.ndarray:
    """
    Compute Aleatoric Uncertainty (AU) and Epistemic Uncertainty (EU) using LogTokU framework.

    Args:
        token_prob: Token probabilities (n_step, n_token) - not used directly but for shape reference
        token_entropy: Token entropy values (n_step, n_token)
        top_10_probs: Top-K logits for each token (n_step, n_token, K) where K=10

    Returns:
        dict: Dictionary containing AU and EU metrics
    """
    metrics = {}

    if top_10_probs is None:
        return metrics

    # Convert to numpy if tensor
    if isinstance(top_10_probs, torch.Tensor):
        top_10_probs = top_10_probs.numpy()

    n_step, n_token, K = top_10_probs.shape

    # Compute AU and EU for each token position
    au_per_token = []
    eu_per_token = []

    for token_idx in range(n_token):
        probs = top_10_probs[:, token_idx, :]  # (n_step, K)

        # Convert logits to evidence parameters (alpha_k) using exponential mapping
        # This treats logits as evidence: higher logits = more evidence
        alphas = probs #np.exp(logits)  # (n_step, K)
        alpha0 = alphas.sum(axis=-1, keepdims=True)  # (n_step, 1) - total evidence

        # Compute Aleatoric Uncertainty (AU) - Equation 3
        # AU captures inherent data ambiguity
        # AU(at) = -sum_k (alpha_k / alpha0) * [psi(alpha_k + 1) - psi(alpha0 + 1)]
        normalized_alphas = alphas / alpha0  # (n_step, K)
        digamma_diff = digamma(alphas + 1) - digamma(alpha0 + 1)  # (n_step, K)
        au = -(normalized_alphas * digamma_diff).sum(axis=-1)  # (n_step,)

        # Compute Epistemic Uncertainty (EU) - Equation 4
        # EU captures model knowledge gaps
        # EU(at) = K / sum_k (alpha_k + 1)
        eu = K / (alphas + 1).sum(axis=-1)  # (n_step,)

        au_per_token.append(au)
        eu_per_token.append(eu)

    # Stack across tokens
    au_per_token = np.stack(au_per_token, axis=-1)  # (n_step, n_token)
    eu_per_token = np.stack(eu_per_token, axis=-1)  # (n_step, n_token)

    # Stack these metrics to be (n_step, n_token, 4)
    # Stack: [AU, EU, log_probs, token_entropy] along last dimension
    log_probs = -np.log(token_prob + 1e-10)  # (n_step, n_token)

    # Stack all 4 metrics: AU, EU, log_probs, token_entropy
    stacked_metrics = np.stack([
        au_per_token,      # (n_step, n_token)
        eu_per_token,      # (n_step, n_token)
        log_probs,         # (n_step, n_token)
        token_entropy      # (n_step, n_token)
    ], axis=-1)  # (n_step, n_token, 4)
...
def load_rollouts(cfg: Config) -> list[Rollout]:
    # Load data
    all_csv = sorted(glob.glob(f"{cfg.dataset.data_path}*.csv"))
    all_rollouts = []
    ...
    for csv_path in tqdm(all_csv, desc="Loading rollouts"):
        ...
        if os.path.exists(pkl_path):
            ...
            # Extract top-10 logits if available
            top_10_probs = torch.tensor(rollout["logits"], dtype=torch.float32) if "logits" in rollout else None
            if top_10_probs is None:
                top_10_probs = torch.tensor(rollout["top10_probs"], dtype=torch.float32) if "top10_probs" in rollout else None
            # Compute AU and EU metrics using LogTokU framework
            au_eu_metrics = None
            if 'action/token_0_prob' in df.columns:
                token_prob = df[[
                    'action/token_0_prob', 'action/token_1_prob', 'action/token_2_prob', 'action/token_3_prob',
                ...
                token_entropy = df[[
                    'action/token_0_entropy', 'action/token_1_entropy', 'action/token_2_entropy',
                    'action/token_3_entropy',
                    'action/token_4_entropy', 'action/token_5_entropy', 'action/token_6_entropy'
                ]].values  # (n_step, n_token)
                au_eu_metrics = compute_au_eu_metrics(token_prob, token_entropy, top_10_probs)
        ...
        r = Rollout(
            ...
            probs=torch.Tensor(rollout.get("probs", None)),
            ...
            probs_features=torch.Tensor(au_eu_metrics),
        )
    ...
    # add all rollouts to the same array - concat all the probs,task_suite_name, task_id ,.. etc
    # all_rollouts = concat_rollouts(all_rollouts)
