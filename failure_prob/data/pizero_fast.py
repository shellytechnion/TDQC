...
from failure_prob.utils.failure_metrics import compute_token_metrics, compute_sample_unc_metrics, \
    compute_stac_metrics_single, compute_stac_metrics
...
def compute_hand_crafted_metrics(
    ...
    '''
    Compute hand-crafted metrics for pi0-fast rollouts.
    Token-level metrics are derived from logits via softmax (mirroring univla),
    producing max/avg token probability and entropy per inference step.
    '''
...
def load_rollouts_from_root(load_root: Path, cfg: Config) -> list[Rollout]:
    ...
    for env_record_path in tqdm(env_record_paths, desc="Loading rollouts"):
        ...
        for policy_record in policy_records:
            ...
            # Extract top-10 probs from action-bin logits (similar to univla.py)
            if "logits" in policy_record:
                logits = torch.from_numpy(policy_record["logits"].astype(np.float32))  # (n_tokens, n_vocab)
                # action_start = policy_record.get("action_start_index_in_vocab", 0)
                # action_end = policy_record.get("action_end_index_in_vocab", logits.shape[-1])
                # token_logits = logits[:, action_start:action_end]  # (n_tokens, n_bins)
                token_prob = torch.softmax(logits, dim=-1)   # (n_tokens, n_bins)
                top_k = torch.topk(token_prob, k=10, dim=-1).values  # (n_tokens, 10)
                top_k_probs_list.append(top_k.numpy())
        ...
        # Pad and stack top_k_probs -> (n_steps, max_tokens, K)
        top_k_tensor = None
        if top_k_probs_list:
            ...
            top_k_tensor = torch.from_numpy(np.stack(padded_top_k, axis=0))  # (n_steps, max_tokens, K)
