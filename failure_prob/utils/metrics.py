    ...
    for split, rollouts_split in rollouts_by_split_name.items():
        ...
        for task_id in task_ids + ["all"]:
            with warnings.catch_warnings():
                ...
                scores = [s[:max(1, r.task_min_step)].max() for s, r in zip(scores_task, rollouts_task)]
                ...
                scores = [s[:max(1, len(r.hidden_states))].max() for s, r in zip(scores_task, rollouts_task)]
                fpr, tpr, thresholds = roc_curve(labels_task, scores)
                roc_auc = auc(fpr, tpr)
                ...
                for q in time_quantiles:
                    scores = [s[:max(1, int(len(r.hidden_states)* q))].max() for s, r in zip(scores_task, rollouts_task)]
                    fpr, tpr, thresholds = roc_curve(labels_task, scores)
                    roc_auc = auc(fpr, tpr)
                    to_be_logged[f'falert_end_roc_auc/{method_name}_{split}_qt_{q}_all_max'] = roc_auc
                    roc_curves_data_end.append((fpr, tpr, split, roc_auc))

                    scores = [s[int((len(r.hidden_states) - 1) * q)] for s, r in zip(scores_task, rollouts_task)]
                    fpr, tpr, thresholds = roc_curve(labels_task, scores)
                    roc_auc = auc(fpr, tpr)
                    to_be_logged[f'falert_end_roc_auc/{method_name}_{split}_qt_{q}_all'] = roc_auc
                    roc_curves_data_end.append((fpr, tpr, split, roc_auc))
    ...
    for split_name in rollouts_by_split_name:
        ...
        for eval_time in EVAL_TIMES:
            if eval_time == "at earliest stop":
                ...
                scores = [s[:max(1, r.task_min_step)].max() for s, r in zip(scores_all, rollouts)]
            elif eval_time == "by final end":
                scores = [s[:max(1, len(r.hidden_states))].max() for s, r in zip(scores_all, rollouts)]
            else:
...
def eval_split_conformal(
    ...
    for eval_time in EVAL_TIMES:
        if eval_time == "at earliest stop":
            ...
            cal_scores = [s[:max(1, r.task_min_step)].max() for s, r in zip(cal_scores_all, cal_rollouts)]
            test_scores = [s[:max(1, r.task_min_step)].max() for s, r in zip(test_scores_all, test_rollouts)]
        elif eval_time == "by final end":
            cal_scores = [s[:max(1, len(r.hidden_states))].max() for s, r in zip(cal_scores_all, cal_rollouts)]
            test_scores = [s[:max(1, len(r.hidden_states))].max() for s, r in zip(test_scores_all, test_rollouts)]
        else:
            raise ValueError(f"Unknown eval_time: {eval_time}")
...
def compute_functional_conformal_band(
    rollouts: list[Rollout],
    scores: list[np.ndarray],
    alpha: float,
    calib_on: str = "neg",
    ...
    """
    Compute conformal prediction band without testing.
    
    Args:
        rollouts: List of calibration rollouts
        scores: List of score sequences for calibration
        alpha: Significance level (e.g., 0.1 for 90% coverage)
        calib_on: "neg" to calibrate on successful rollouts, "pos" to calibrate on failed rollouts
        align_method: "extend" to pad shorter sequences with last value
    ...
    if align_method == "extend":
        ...
        scores_aligned = []
        for s in scores:
            scores_aligned.append(np.pad(s, (0, max_length - len(s)), mode='edge'))
    ...
    # Filter scores based on calibration type
    if calib_on == "neg":  # Calibration on successful rollouts
        lower_bound = False
        cal_scores_used = [s for s, r in zip(scores_aligned, rollouts) if r.episode_success == 1]
    else:  # Calibration on failed rollouts
        lower_bound = True
        cal_scores_used = [s for s, r in zip(scores_aligned, rollouts) if r.episode_success == 0]
    
    if len(cal_scores_used) == 0:
        raise ValueError(f"No calibration data found for calib_on='{calib_on}'. "
                       f"Total rollouts: {len(rollouts)}, "
                       f"Success count: {sum(r.episode_success == 1 for r in rollouts)}, "
                       f"Failure count: {sum(r.episode_success == 0 for r in rollouts)}")
    
    # Split calibration scores into two sets (for regression and modulation)
    ...
    if len(cal_scores_used) == 1:
        ...
        n_cal_1 = int(len(cal_scores_used) * 0.3)  # 30% for regression
...
def eval_functional_conformal(
    ...
    for calib_on in calib_ons:
        if calib_on == "neg": # Calibration on the successful rollouts
            ...
            lower_bound = True
            cal_scores_used = [s for s, r in zip(cal_scores_all[0], cal_rollouts) if r.episode_success == 1]
...
def get_ece(conf, labels, n_bins=15, p=1):
    ...
    Args:
        conf: Confidence scores (probabilities)
        labels: Ground truth binary labels
        n_bins: Number of bins for calibration
        p: Norm parameter (typically 1 for ECE, 2 for RMS)
    
    Returns:
        ECE value
    """
    try:
        # Try using calibration library if available
        import calibration as cal
        ece = cal.lower_bound_scaling_ce(
            conf, labels, p=p, debias=False, num_bins=n_bins, 
            binning_scheme=cal.get_equal_bins, mode='top-label'
        )
    except ImportError:
        # Fallback: Simple ECE implementation
...
def eval_ece_brier_score(
    ...
    """
    Evaluate ECE and Brier score at different time quantiles.
    
    Args:
        rollouts_by_split_name: Dict of rollouts by split
        scores_by_split_name: Dict of scores by split
        method_name: Name of the method
        time_quantiles: List of time quantiles to evaluate (e.g., [0.25, 0.5, 0.75, 1.0])
        n_cal_bins: Number of bins for ECE calculation
    
    Returns:
        Dictionary of logs to be sent to wandb
    """
    to_be_logged = {}
    time_quantiles = list(np.arange(100) / 100)
    
    for split_name in rollouts_by_split_name:
        rollouts = rollouts_by_split_name[split_name]
        scores_all = scores_by_split_name[split_name]
        scores_concat = np.concatenate([np.asarray(s).ravel() for s in scores_all])
        if scores_concat.max() > 1.0 or scores_concat.min() < 0.0:
            print(f"Warning: Scores for split {split_name} are not in [0, 1]. Normalizing to [0, 1]. for ECE/Brier evaluation.")
            # s_min, s_max = scores_concat.min(), scores_concat.max()
            scores_all = [(np.asarray(s) - s.min()) / (s.max() - s.min() + 1e-5) for s in scores_all]
        # Get ground truth labels (1 = failure, 0 = success)
        labels = np.array([1 - r.episode_success for r in rollouts])
        
        ece_results = []
        brier_results = []
        
        for quantile in time_quantiles:
            # Collect probabilities at this time quantile
            all_probs = []
            all_labels = []
            ...
            for i, r in enumerate(rollouts):
                ...
                # Get the timestep corresponding to this quantile
                timestep = int(quantile * T)
                if timestep >= T:
                    timestep = T - 1
                
                if timestep >= 0:
                    # Use max score up to this timestep (max-so-far strategy)
                    # prob = np.max(score[:timestep + 1])
                    prob = score[timestep]
            ...
            # Compute ECE at this quantile
            ece = get_ece(all_probs, all_labels, n_bins=n_cal_bins)
            
            # Compute Brier score at this quantile
        ...
        # Log as wandb Tables (for tabular data)
        to_be_logged[f"calibration/{method_name}_ece_table_{split_name}"] = wandb.Table(dataframe=ece_df)
        to_be_logged[f"calibration/{method_name}_brier_table_{split_name}"] = wandb.Table(dataframe=brier_df)
        
        # Log individual data points for wandb line plots
        # This will create interactive line graphs in wandb
        for idx, row in ece_df.iterrows():
            to_be_logged[f"calibration_curves/{method_name}_ece_{split_name}"] = wandb.plot.line(
                wandb.Table(dataframe=ece_df),
                "quantile",
                "ece",
                title=f"ECE vs Time ({split_name})"
            )
        
        for idx, row in brier_df.iterrows():
            to_be_logged[f"calibration_curves/{method_name}_brier_{split_name}"] = wandb.plot.line(
                wandb.Table(dataframe=brier_df),
                "quantile",
                "brier_score",
                title=f"Brier Score vs Time ({split_name})"
            )
        
        # Also log overall metrics at task_min_step - 1 (at earliest stop)
        scores_at_stop = [s[r.task_min_step - 1] for s, r in zip(scores_all, rollouts)]
        ece_at_stop = get_ece(np.array(scores_at_stop), labels, n_bins=n_cal_bins)
        brier_at_stop = np.mean((np.array(scores_at_stop) - labels) ** 2)
        
        to_be_logged[f"calibration/{method_name}_ece_at_stop_{split_name}"] = ece_at_stop
        to_be_logged[f"calibration/{method_name}_brier_at_stop_{split_name}"] = brier_at_stop
