...
declare -a QVALUE_THRESH=(0.3 0.35 0.4 0.5 0.6)
...
for n in "${N_CANDIDATES[@]}"; do
    for temp in "${TEMPERATURES[@]}"; do
		...
		CUDA_VISIBLE_DEVICES=${CUDA_DEVICE} python openvla/experiments/robot/libero/run_libero_eval_with_qnetwork.py \
		    ...
		    --run_id_note "grid_n${n}_temp${temp}_BCE_thresh${thresh}" \
		    --calibration_seed ${SEED} \
		    --conformal_alpha ${ALPHA} \
		    --save_videos True \
		    --save_root "/mnt/pub/shellyf/SAFE/rollouts_grid_n${n}_temp${temp}_BCE_thresh${thresh}" \
